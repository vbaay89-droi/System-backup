...
// 1. ADD EVENT (with Manager Assignment)
if (isset($_POST['add_event'])) {
    $game_id = $_POST['game_id'];
    $event_name = $_POST['event_name'];
    $assigned_user_id = !empty($_POST['assigned_user_id']) ? (int)$_POST['assigned_user_id'] : NULL;

    $stmt = $conn->prepare("INSERT INTO game_events (game_id, event_name, assigned_user_id) VALUES (?, ?, ?)");
    $stmt->bind_param("isi", $game_id, $event_name, $assigned_user_id);
    
    if ($stmt->execute()) {
        $_SESSION['message'] = "Game Event '{$event_name}' added successfully.";
...
if (isset($_POST['edit_event'])) {
    ...
    $assigned_user_id = !empty($_POST['edit_assigned_user_id']) ? (int)$_POST['edit_assigned_user_id'] : NULL;

    $stmt = $conn->prepare("UPDATE game_events SET game_id = ?, event_name = ?, assigned_user_id = ? WHERE event_id = ?");
    $stmt->bind_param("isii", $game_id, $event_name, $assigned_user_id, $event_id);
    
    if ($stmt->execute()) {
        $_SESSION['message'] = "Game Event updated successfully.";
        $_SESSION['message_type'] = 'success';
    } else {
...
// --- FETCH DATA (READ) ---
// Fetch Level 1 Games for the dropdown
$games = [];
...
// Fetch Event Managers for the dropdown
$event_managers = [];
$sql_managers = "SELECT id, full_name FROM users WHERE role = 'Event Manager' AND status = 'active' ORDER BY full_name";
$result_managers = $conn->query($sql_managers);
if ($result_managers) {
    $event_managers = $result_managers->fetch_all(MYSQLI_ASSOC);
}

// Fetch existing Level 2 Events to display in the table
$game_events = [];
$sql_events = "SELECT ge.*, g.game_name, u.full_name as manager_name 
               FROM game_events ge 
               JOIN games g ON ge.game_id = g.game_id
               LEFT JOIN users u ON ge.assigned_user_id = u.id
               ORDER BY g.game_name, ge.event_name";
$result = $conn->query($sql_events);
if ($result) {
    $game_events = $result->fetch_all(MYSQLI_ASSOC);
}
...
<head>
    ...
    <title>Manage Game Events (L2) - Admin Dashboard</title>
    ...
    <style>
        ...
        .main-content {
            ...
            opacity: 0; /* Added for fade-in */
            transform: translateY(10px); /* Added for fade-in */
        }
...
<body>
    ...
    <div class="main-content">
        <div class="container-fluid">
            <h1 class="section-title mb-4">Manage Game Events (Level 2)</h1>
            ...
            <div class="card">
                <div class="card-header card-header-flex">
                    <h5 class="mb-0">All Events (e.g., Runs & Jumps)</h5>
                        ...
                        <table class="table table-hover align-middle">
                            <thead class="table-light">
                                <tr>
                                    <th>Game (L1)</th>
                                    <th>Event Name (L2)</th>
                                    <th>Assigned Manager</th> <th>Actions</th> </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($game_events as $event): ?>
                                <tr>
                                    <td><?= htmlspecialchars($event['game_name']) ?></td>
                                    <td><?= htmlspecialchars($event['event_name']) ?></td>
                                    <td><?= !empty($event['manager_name']) ? htmlspecialchars($event['manager_name']) : '<span class="text-muted">None</span>' ?></td>
                                    <td>
                                        <button class="btn btn-sm btn-outline-primary edit-btn"
                                            ...
                                            data-manager-id="<?= $event['assigned_user_id'] ?? '' ?>"
                                            title="Edit">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Add New Event (L2)</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="Manage_Game_Events.php" method="POST">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label for="event_name" class="form-label">Event Name (L2)</label>
                            <input type="text" class="form-control" id="event_name" name="event_name" placeholder="e.g., Runs & Jumps" required>
                        </div>
                        <div class="mb-3">
                            <label for="assigned_user_id" class="form-label">Assign Event Manager</label>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="Manage_Game_Events.php" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            <label for="edit_game_id" class="form-label">Parent Game (L1)</label>
                            <select class="form-select" id="edit_game_id" name="edit_game_id" required>
                        ...
                        <div class="mb-3">
                            <label for="edit_event_name" class="form-label">Event Name (L2)</label>
                            <input type="text" class="form-control" id="edit_event_name" name="edit_event_name" required>
                        </div>
                        <div class="mb-3">
                            <label for="edit_assigned_user_id" class="form-label">Assign Event Manager</label>
                            <select class="form-select" id="edit_assigned_user_id" name="edit_assigned_user_id">
                                <option value="">-- None --</option>
                                <?php foreach ($event_managers as $manager): ?>
                                <option value="<?= $manager['id'] ?>"><?= htmlspecialchars($manager['full_name']) ?></option>
                                <?php endforeach; ?>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="Manage_Game_Events.php" method="POST">
                    ...
                    <div class="modal-body">
                        <p>Are you sure you want to delete this event: <strong id="delete_event_name"></strong>?</p>
                        <p class="text-danger">This will delete all associated categories (e.g., 100m Dash) and their results.</p>
                    </div>
                    <div class="modal-footer">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // --- Page-specific JS for this page (Merged) ---
            ...
            if(editEventModal) {
                editEventModal.addEventListener('show.bs.modal', function (event) {
                    ...
                    document.getElementById('edit_event_id').value = button.getAttribute('data-id');
                    document.getElementById('edit_event_name').value = button.getAttribute('data-name');
                    document.getElementById('edit_game_id').value = button.getAttribute('data-game-id');
                    document.getElementById('edit_assigned_user_id').value = button.getAttribute('data-manager-id');
                });
            }
            
            // JS to populate the delete modal
            const deleteEventModal = document.getElementById('deleteEventModal');
            if(deleteEventModal) {
                deleteEventModal.addEventListener('show.bs.modal', function (event) {
                    const button = event.relatedTarget;
                    document.getElementById('delete_event_id').value = button.getAttribute('data-id');
                    document.getElementById('delete_event_name').textContent = button.getAttribute('data-name');
                });
            }
